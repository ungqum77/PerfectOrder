...
export const mockSupabase = {
    ...
    auth: {
        ...
        signIn: async (email: string, password: string): Promise<{ user: User | null, error: string | null }> => {
            ...
            if (typeof window !== 'undefined') localStorage.setItem(STORAGE_KEYS.SESSION, JSON.stringify(user));
            return { user, error: null };
        },
    ...
    db: {
        ...
        orders: {
            init: () => {
                ...
                if (typeof window !== 'undefined' && !localStorage.getItem(STORAGE_KEYS.ORDERS)) {
